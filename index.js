...
const flockUrlRouter = {
    /**
     * Keys are flockUrls, values are instances of MessageQueue
     *
    ...
    /**
     * Add the message to appropriate message queue (each flock URL has own independent message enqueing).
     *
     * @param {Message} message
     */
    addMessage: function (message) {
        const processName = message.name;
        const flockUrl = moduleConfig['flock_url-' + processName] || moduleConfig['flock_url'];

        if (!flockUrl) {
            return;
            // No flock URL defined for this process and no global flock URL exists.
        }

        if (!this.messageQueues[flockUrl]) {
            // Init new messageQueue to different flock URL.

            // Resolve configuration parameters.
            const configProperties = ['username', 'servername', 'buffer', 'flock_url', 'buffer_seconds', 'buffer_max_seconds', 'queue_max'];
            const config = {};
            ...
            this.messageQueues[flockUrl] = new MessageQueue(config);
        }

        this.messageQueues[flockUrl].addMessageToQueue(message);
